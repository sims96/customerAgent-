...
(function() {
  ...
  window.mobileChat = {
      ...
      if (!document.getElementById('mobile-chat-view')) {
          ...
          <div id="mobile-chat-view" class="mobile-view">
            <div class="mobile-header brand-gradient">
              ...
              <div class="flex space-x-2">
                ...
                <button id="mobile-reset-btn" class="bg-red-600 hover:bg-red-700 text-white h-8 w-8 rounded-full flex items-center justify-center text-sm">
        ...
        const mobileStyles = `
          <style id="mobile-chat-styles">
            ...
            .mobile-header .flex.space-x-2 {
              display: flex;
              gap: 0.4rem; /* Tighter spacing for the buttons */
            }
            
            #mobile-reset-btn {
              /* Make the button stand out with a red background */
            ...
            /* If the header gets crowded on very small screens */
    ...
    async handleResetConversation() {
      ...
      try {
        window.logToConsole(`Resetting conversation controls from mobile: ${this.activeUserId}`);
        const resetBtn = document.getElementById('mobile-reset-btn');
        resetBtn.disabled = true;
        resetBtn.innerHTML = '<div class="loader"></div>';
        
        // Force reset the conversation handling status
        await window.api.request(
          `/api/conversation/status?userId=${encodeURIComponent(this.activeUserId)}`,
          'PUT',
          {
        ...
        window.logToConsole('Conversation controls reset successfully from mobile');
      } catch (error) {
        window.logToConsole(`Failed to reset conversation from mobile: ${error.message}`, true);
        document.getElementById('mobile-reset-btn').disabled = false;
        document.getElementById('mobile-reset-btn').innerHTML = '<i class="fas fa-redo-alt"></i>';
      }
    },
